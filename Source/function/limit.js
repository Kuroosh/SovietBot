 ...
 * @returns {boolean}
 */
export const isLimit = (userId, _dir, limitCount, isPremium, isOwner) => {
 ...
 * @param {boolean} isOwner 
 */
export const addLimit = (userId, _dir, isPremium, isOwner) => {
    if (isPremium || isOwner) return false
    let pos = null
 ...
 * @returns {number}
 */
export const getLimit = (userId, _dir, limitCount) => {
    let pos = null
    let found = false
    Object.keys(_dir).forEach((i) => {
        if (_dir[i].id === userId) {
